...
// Ô nhớ 30: Preday
// Ô nhớ 80: Countday
// Ô nhớ 100: Current savings
// Ô nhớ 120: State savings daily
// ************** //

// *** UART *** //
// *** Pin thứ 2 DFPlayer mini -> Trở 1k -> Tx216 Arduino Mega *** //
// *** Pin thứ 2 DFPlayer mini -> Trở 1k -> Rx217 Arduino Mega *** //
// *** UAT ESP32-Cam -> Tx119 Arduino Mega *** //
// ************** //

// *** Busy Pin DFPlayer mini -> 49 Arduino Mega *** //
// *** Chân giữa PIR -> 47 Arduino Mega *** //
// *** Speaker+ -> SPK_1 DFPlayer mini *** //
// *** Speaker- -> SPK_2 DFPlayer mini *** //
      ...
      while (isDays == false) {
        ...
        if (customKey == '#' && days != "") {
          ...
          if (Serial1.available() > 0) {
            String date = Serial1.readStringUntil('\n');
            if (date.length() <= 27) {
...
String currentSavings = "0";
unsigned long int countSavings = 0;
int pir_PIN = 21;
int pir_Val = 0;
String stateSavingsDaily = readStringFromEEPROM(120);
int busy_PIN = 49;
int busy_VAL = 0;
  ...
  if (Serial1.available() > 0) {
    ...
    if (date.length() <= 27) {
      ...
      if (preDays != partBeforeComma) {    // Nếu ngày đã lưu trước đây khác với ngày hiện tại (Sang ngày mới)
        stateSavingsDaily = "No";
        writeStringToEEPROM(120, stateSavingsDaily);
...
void displayDaysWthPrint() {
  if (Serial1.available() > 0) {
    ...
    if (date.length() <= 27) {
      ...
      Serial.println(preDays);
      Serial.println(partBeforeComma);
      Serial.println(readStringFromEEPROM(120));
    }
  }
}
...
void saveByDays() {
  ...
  tft.setTextColor(tft.color565(29, 47, 70));
  // tft.print("Savings: ");
  currentSavings = readStringFromEEPROM(100);
  currentSavings = String(map(currentSavings.toInt(), 0, readStringFromEEPROM(10).toInt(), 0, 170));
  tft.fillRoundRect(105, 200, currentSavings.toInt(), 10, 15, tft.color565(156, 128, 63));
  currentSavings = readStringFromEEPROM(100);
  // Serial.println(currentSavings.length());
  if (currentSavings.length() == 7) {
    tft.setCursor(105, 175);
  } else if (currentSavings.length() == 6) {
    tft.setCursor(105, 175);
  } else if (currentSavings.length() == 5) {
    tft.setCursor(110, 175);
  } else if (currentSavings.length() == 4) {
    tft.setCursor(110, 175);
  } else if (currentSavings.length() == 1) {
    tft.setCursor(140, 175);
  }
  tft.println("Curr: " + formatWithDots(currentSavings) + " d");
  ...
  while (readStringFromEEPROM(10) != "" && readStringFromEEPROM(20) != "") {
    ...
    while (countPr <= 500) {
      String date = "";
      if (Serial1.available() > 0) {
        date = Serial1.readStringUntil('\n');
      }
      countPr++;
      Serial.println(countPr);
      Serial.println(date);
    }
    // if (countSavings == currentSavings.toInt() && stateSavingsDaily == "No") {
    //   pir_Val = digitalRead(pir_PIN);
    //   if (pir_Val == 1) {
    //     myDFPlayer.volume(30);
    //     randomPlaySound = random(5, 8);
    //     myDFPlayer.play(randomPlaySound);
    //     delay(500);
    //     busy_VAL = digitalRead(busy_PIN);
    //     while (busy_VAL == 0) {
    //       busy_VAL = digitalRead(busy_PIN);
    //       Serial.println(busy_VAL);
    //       displayDays();
    //     }
    //   }
    // }
    ...
    displayDays();
    // --- Display Target --- //
    // --- Tính toán tiền nhập vào --- //
    // countSavings = readStringFromEEPROM(100).toInt() + duLieuTuESP32Cam.toInt();
    // --- Xác định điều kiện --- //
    currentSavings = readStringFromEEPROM(100);
    if (countSavings > currentSavings.toInt() && countSavings < readStringFromEEPROM(10).toInt()) {
      stateSavingsDaily = "Yes";
      writeStringToEEPROM(120, stateSavingsDaily);
      currentSavings = String(countSavings);
      writeStringToEEPROM(100, currentSavings);
      currentSavings = String(map(currentSavings.toInt(), 0, readStringFromEEPROM(10).toInt(), 0, 170));
      tft.fillRoundRect(105, 200, currentSavings.toInt(), 10, 15, tft.color565(156, 128, 63));
      currentSavings = String(countSavings);
      if (currentSavings.length() == 7) {
      ...
      tft.setTextColor(tft.color565(29, 47, 70), WHITE);
      tft.println("Curr: " + formatWithDots(currentSavings) + " d");
    } else if (countSavings > currentSavings.toInt() && countSavings >= readStringFromEEPROM(10).toInt()) {
      stateSavingsDaily = "Yes";
      writeStringToEEPROM(120, stateSavingsDaily);
      ...
      writeStringToEEPROM(100, currentSavings);
      // currentSavings = String(map(currentSavings.toInt(), 0, readStringFromEEPROM(10).toInt(), 0, 170));
      tft.fillRoundRect(105, 200, 170, 10, 15, tft.color565(156, 128, 63));
      if (currentSavings.length() == 7) {
        tft.setCursor(105, 175);
      } else if (currentSavings.length() == 6) {
        tft.setCursor(105, 175);
...
void saveByTarget() {
  ...
  tft.setTextSize(2.9);
  tft.setTextColor(WHITE);
  tft.println(formatWithDots(readStringFromEEPROM(10)) + " d");
  tft.fillRoundRect(75, 125, 200, 125, 0, WHITE);

  tft.drawRoundRect(75, 148, 230, 60, 0, tft.color565(255, 122, 83));
  currentSavings = readStringFromEEPROM(100);
  currentSavings = String(map(currentSavings.toInt(), 0, readStringFromEEPROM(10).toInt(), 0, 230));
  Serial.println("Current Savings: " + currentSavings);
  tft.fillRoundRect(80, 155, 210, 50, 0, WHITE);
  tft.fillRoundRect(75, 148, currentSavings.toInt(), 60, 0, tft.color565(255, 122, 83));
  currentSavings = readStringFromEEPROM(100);
  if (currentSavings.length() == 7) {
    tft.setCursor(95, 167);
  } else if (currentSavings.length() == 6) {
    tft.setCursor(107, 167);
  } else if (currentSavings.length() == 5) {
    tft.setCursor(130, 167);
  } else if (currentSavings.length() == 4) {
    tft.setCursor(130, 167);
  } else if (currentSavings.length() == 1) {
    tft.setCursor(142, 167);
  }
  showBMP("Coin_set_target_icon_45px.bmp", 19, 157);
  tft.setTextSize(3);
  tft.setTextColor(tft.color565(29, 47, 70));
  tft.println(formatWithDots(currentSavings) + " d");
  ...
  while (readStringFromEEPROM(10) != "" && readStringFromEEPROM(20) == "") {
    ...
    if (countSavings > currentSavings.toInt() && countSavings < readStringFromEEPROM(10).toInt()) {
      ...
      currentSavings = String(map(currentSavings.toInt(), 0, readStringFromEEPROM(10).toInt(), 0, 230));
      tft.fillRoundRect(80, 155, 210, 50, 0, WHITE);
      tft.fillRoundRect(75, 148, 230, 60, 0, tft.color565(255, 122, 83));
      currentSavings = String(countSavings);
      if (currentSavings.length() == 7) {
        tft.setCursor(95, 167);
      } else if (currentSavings.length() == 6) {
...
void setup(void) {
  ...
  Serial2.begin(9600);
  if (!myDFPlayer.begin(Serial2, false)) {  //Use serial to communicate with mp3.
  ...
  while (Serial1.available() == 0) {
    ...
    Serial.println("Waiting to connect ESP32-Cam.....");
    date = Serial1.readStringUntil('\n');
    if (date == "No time available (yet)" || date == " CONNECTED") {
      continue;
    }
    if (date.length() <= 27) {
      int commaIndex = date.indexOf(',');
  ...
  writeStringToEEPROM(100, "0");
  writeStringToEEPROM(120, "No");
  Serial.println(AVR_EEPROM.read_2_byte(80));
  Serial.println(readStringFromEEPROM(120));
  attachInterrupt(digitalPinToInterrupt(pir_PIN), notify, RISING);
